...
class CliffWalkingEnv(discrete.DiscreteEnv): #Inherets from Gym's DiscreteEnv

    metadata = {'render.modes': ['human', 'ansi', 'rgb_array']}

    def _limit_coordinates(self, coord):
        # rounds down the coordinates to keep them inside the map or sth
    ...
    def _calculate_transition_prob(self, current, delta):
        ...
        # Punishment of -100 when reaching a cliff and a reward of -1 when reaching or NOT reaching goal
        is_done = self._cliff[tuple(new_position)] or (tuple(new_position) == (3,11))
        return [(1.0, new_state, reward, is_done)]

    def __init__(self):
        self.name = "Cliff Walking Environment"
        self.shape = (4, 12) # A 4x12 shape of the maze

        nS = np.prod(self.shape) # State space = 48 states
        nA = 4 # 4 distinct actions
        ...
        """ P is a dictionary of dicts of lists, where
          P[s] is a dictionary
          P[s][a] is a list
