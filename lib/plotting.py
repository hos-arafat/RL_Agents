...
EpisodeStats = namedtuple("Stats",["name","label","episode_lengths", "episode_rewards", "episode_running_variance"])
...
def plot_episode_stats(stats, smoothing_window=10, hideplot=False):
    ...
    plt.title(stats.name + "\n" + stats.label + " (Episode Length over Time)")
    if hideplot:
        plt.close(fig1)
    else:
        plt.show(fig1)
    ...
    plt.title(stats.name + "\n" + stats.label + " (Episode Reward over Time)")
    if hideplot:
        plt.close(fig2)
    else:
        plt.show(fig2)
